...
function eventRouter(settings) {
    ...
    router.get('/', ensureAuthenticatedUser, userEnsure2fa, async (req, res) => {
        ...
        res.render('evenementen', { settings, nonce: res.locals.cspNonce, user: req.user ?? undefined, events: events });
    });


    router.get('/calendar', ensureAuthenticatedUser, userEnsure2fa, async (req, res) => {
        res.render('evenementen/calendar', { settings, nonce: res.locals.cspNonce, user: req.user ?? undefined });
    });
    ...
    router.get('/:EventID', ensureAuthenticatedUser, userEnsure2fa, async (req, res) => {
        ...
        res.render('evenementen/details', { settings, nonce: res.locals.cspNonce, user: req.user ?? undefined, eventDetails: eventDetails, attendance: attendance, cancelDate: CancelableUntil });
    });
    ...
    router.get('/:EventID/participation/cancel', async (req, res) => {
        ...
        res.render('evenementen/tickets', { settings, nonce: res.locals.cspNonce, user: req.user ?? undefined, attendees: attendees, EventID: req.params.EventID });
    });
